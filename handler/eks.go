...
func EksData(token string) {
	...
	for i := range obj {
		...
		sqlStatement := `
			INSERT INTO ekspipeline (id, sha, ref, status, web_url, kibana_url)
			VALUES ($1, $2, $3, $4, $5, $6)
		...
		fmt.Println("New record ID for EKS Pipeline:", id)

		// Push Eks jobs data to Database
		for j := range jobsdata {
			...
			fmt.Println("New record ID for EKS Jobs: ", id)
		}
	}
}

// QueryEksData fetch the pipeline data as well as jobs data for eks platform
func QueryEksData(datas *dashboard) error {
	pipelinerows, err := database.Db.Query(`SELECT * FROM ekspipeline ORDER BY id DESC`)
	if err != nil {
		fmt.Println(err)
	}
