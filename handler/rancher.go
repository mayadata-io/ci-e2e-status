...
//RancherPipelineHandler to export data in /api/pipeline/Rancher endpoint
func RancherPipelineHandler(w http.ResponseWriter, r *http.Request) {
	// Allow cross origin request
	(w).Header().Set("Access-Control-Allow-Origin", "*")
	datas := dashboard{}
	err := RancherQueryPipelineData(&datas)
	if err != nil {
...
// RancherQueryPipelineData fetch the pipeline data as well as jobs data form Packet table of database
func RancherQueryPipelineData(datas *dashboard) error {
	// Select all data from packetpipeline table of DB
	query := fmt.Sprintf("SELECT pipelineid,sha,ref,status,web_url,author_name,author_email,message,percentage_coverage FROM rancher_pipeline ORDER BY build_pipeline_id DESC")
	...
	for pipelinerows.Next() {
		...
		jobsquery := fmt.Sprintf("SELECT pipelineid,id,status,stage,name,ref,created_at,started_at,finished_at FROM rancher_pipeline_jobs WHERE pipelineid = $1 ORDER BY id;")
