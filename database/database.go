...
func createTable() {
	...
	pipeline := []string{"packet_pipeline_v11", "packet_pipeline_v12", "packet_pipeline_v13", "openshift_pipeline"}
	pipelineJobs := []string{"packet_jobs_v11", "packet_jobs_v12", "packet_jobs_v13", "openshift_jobs"}
	...
	query := fmt.Sprintf("CREATE TABLE IF NOT EXISTS build_pipeline(project VARCHAR, id INT PRIMARY KEY, sha VARCHAR, ref VARCHAR, status VARCHAR, web_url VARCHAR, packet_v11_pid VARCHAR, packet_v12_pid VARCHAR, packet_v13_pid VARCHAR, openshift_pid VARCHAR);")
	value, err := Db.Query(query)
	if err != nil {
		glog.Error(err)
	}
